...
async function saveProduct() {
  // Prevent double-click / duplicate product creation.
  ...
  const sell = parseFloat(
    document.getElementById('product-sell').value.replace(',', '.')
  );
  ...
  if (isNaN(purchase) || purchase < 0) {
    toast('Неверная закупочная цена', 'error');
    return;
  }

  if (isNaN(sell) || sell < 0) {
    toast('Неверная цена продажи', 'error');
    return;
  }
  ...
  // Lock immediately so a second tap cannot start another request.
  ...
  try {
    ...
    // Unlock only after the request has finished.
